...
const FindEmployees = () => {
    ...
    useEffect(() => {
        const getSummaries = async () => {
            try {
                const resp = await (hasPhoto ? fetchFilteredSummaries(currentPage, hasPhoto, pageSize) : fetchAllSummaries(currentPage, pageSize));
    ...
    return (
        <>
            <Header/>
            <Lobby/>
            <Candidates hasPhoto={hasPhoto} onHasPhotoChange={handleWithPhotoChanged}/>
            <CV summaries={summaries} totalSummaryCount={totalSummaryCount} currentPage={currentPage} pageSize={pageSize}
